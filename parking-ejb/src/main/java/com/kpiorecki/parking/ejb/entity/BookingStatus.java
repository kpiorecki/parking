...
public enum BookingStatus {
	...
	public static void validateLowerStatus(BookingStatus status, BookingStatus border) {
		if (status == null) {
			throw new DomainException("invalid booking status - null");
		}
		if (status.compareTo(border) >= 0) {
			throw new DomainException(String.format("invalid booking status - %s is not lower than %s", status, border));
		}
	}
